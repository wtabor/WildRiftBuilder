...
export const metadata: Metadata = {
  title: "Design directions · Wild Rift Builder",
  description:
    "Explore the alternate interface directions for the Wild Rift Builder — one shared stat engine, several distinct layouts.",
};
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#070b16] font-sans text-aurora-ink">
      ...
      <div className="relative mx-auto max-w-6xl px-5 py-12 sm:py-20">
        ...
        <div className="max-w-3xl">
          <Link
            href="/"
            className="mb-5 inline-flex items-center gap-1.5 text-sm text-aurora-mute transition hover:text-aurora-ink"
          >
          ...
          <p className="mt-4 text-lg text-aurora-mute">
            One accurate stat &amp; build engine, several distinct interfaces. The{" "}
            <strong className="text-aurora-ink">Meta</strong> direction is now the live builder at
            the home page; the others below are alternate explorations. Your champion, level, and
            items are shared across all of them, so you can flip between them with the same build and
            judge the feel.
          </p>
        </div>
...
function DesignPreview({ id, accent, accent2 }: { id: string; accent: string; accent2: string }) {
  if (id === "meta") {
    ...
    return (
      <div className="relative h-36 overflow-hidden rounded-xl bg-[#faf9f6]">
        <div className="flex h-full">
          <div className="flex w-8 flex-col items-center gap-1.5 border-r border-[#e6e3db] bg-white py-2">
            <span className="h-5 w-5 rounded-lg" style={{ background: `linear-gradient(135deg, ${accent}, ${accent2})` }} />
            <span className="h-4 w-4 rounded-lg bg-[#ece9e2]" />
          </div>
          <div className="flex-1 space-y-1.5 p-2.5">
            <div className="flex items-center gap-1.5">
              <span className="h-2.5 w-1 rounded-full" style={{ background: accent }} />
              <span className="h-2 w-20 rounded bg-[#1a1a1a]/80" />
            </div>
            <div className="flex gap-1.5">
              {Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="flex-1 rounded-lg border border-[#e6e3db] bg-white px-1 py-1">
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="h-5 rounded-lg border border-[#e6e3db] bg-white" />
              ))}
            </div>
          </div>
        </div>
